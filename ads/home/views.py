...
from .models import Confirm, Password, Referral
from dash.models import Yt, Person, Viewed
import datetime
import razorpay
    ...
    if not Confirm.objects.filter(email=uname).exists():
        ...
        if request.method == 'POST':
            ...
            if otp == old_otp:
                ...
                user.save()
                per = Person.objects.create(username=confirm_email.username, points=100)
...
def referred_register(request, ref):
    ...
    if request.method == 'POST':
        ...
        if password1 == password2:
            if User.objects.filter(username=username).exists():
                messages.info(request, 'Username Taken')
                return redirect("referred_register", ref)
                ...
                if Confirm.objects.filter(email=email).exists():
                    ...
                    send_mail(subject, message, from_email, to_list, fail_silently=True)
                    messages.info(request, "An Account Confirmation email has been sent to "+confirm_user.email+". Please Enter the code here.")
                    return redirect("referred_confirm_email", email, ref)
                else:
                    digits = "0123456789"
                    otp = ""
                    for i in range(6):
...
def referred_confirm_email(request, uname, ref):
        ...
        if request.method == 'POST':
            ...
            if otp == old_otp:
                ...
                if User.objects.filter(username=ref).exists():
                    reff = Referral.objects.create(username=ref, referred_username=confirm_email.username)
                    reff.save()
                    pers = Person.objects.filter(username=ref).first()
                    pers.points += 500
                    pers.save()
                messages.info(request, "Email confirmed successfully!")
                messages.info(request, "Login to continue.")
                ...
                if confirm_email.attempts < 4:
                    ...
                    messages.info(request, "Incorrect Otp, Try Again.")
                    messages.info(request, str((5-confirm_email.attempts))+ " attempts left.")
                    return redirect("referred_confirm_email", uname, ref)
                else:
                    confirm_email.attempts = 0
                    confirm_email.save()
                    messages.info(request, "Maximum Attempts held for this confirmation code. We've sent a new Confirmation code to "+confirm_email.email+". Please enter the new Code.")
                    return redirect("referred_resend_code", confirm_email.email, ref)

        

        return render(request, "ref_confirm_email_otp.html", {'confirm_email':confirm_email, 'ref':ref})


def referred_resend_code(request, uname, ref):
    if Confirm.objects.filter(email=uname).exists():
        confirm_user = Confirm.objects.get(email=uname)
        digits = "0123456789"
        otp = ""
